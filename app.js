...
function toggleRapidKey() {
  const input = document.getElementById("rapidapi-key");
  input.type = input.type === "password" ? "text" : "password";
}
...
function salvarConfig() {
  try {
    ...
    localStorage.setItem("rapidapi_key",    document.getElementById("rapidapi-key").value.trim());
...
// =============================================
// API-FOOTBALL — BUSCA DE DADOS AO VIVO
// =============================================

const RAPIDAPI_HOST = "api-football-v1.p.rapidapi.com";
const RAPIDAPI_BASE = "https://api-football-v1.p.rapidapi.com/v3";

// Variáveis de controle do auto-update
let autoUpdateInterval = null;
let autoUpdateCountdown = null;
let ultimoFixtureId = null;
let ultimoJogoNome = null;

// Busca jogos ao vivo pela API-Football
async function buscarJogosAoVivo(rapidApiKey) {
  const res = await fetch(`${RAPIDAPI_BASE}/fixtures?live=all`, {
    headers: {
  ...
  if (!res.ok) {
    ...
    throw new Error(`Erro API-Football: ${err.message || res.statusText}`);
  }
  const data = await res.json();
  return data.response || [];
}

// Busca estatísticas de um jogo pelo fixture ID
async function buscarEstatisticas(rapidApiKey, fixtureId) {
  const res = await fetch(`${RAPIDAPI_BASE}/fixtures/statistics?fixture=${fixtureId}`, {
    headers: {
      "x-rapidapi-key":  rapidApiKey,
      "x-rapidapi-host": RAPIDAPI_HOST
    }
...
// Busca eventos do jogo (gols, cartões, substituições)
async function buscarEventos(rapidApiKey, fixtureId) {
  const res = await fetch(`${RAPIDAPI_BASE}/fixtures/events?fixture=${fixtureId}`, {
    headers: {
      "x-rapidapi-key":  rapidApiKey,
      "x-rapidapi-host": RAPIDAPI_HOST
    }
...
// Encontra o jogo correto na lista de jogos ao vivo
function encontrarJogo(jogosAoVivo, nomeJogo) {
  const partes = nomeJogo.toLowerCase().split(/x|vs/).map(p => normalizarNome(p.trim()));
  const timeCasa = partes[0] || "";
  const timeFora = partes[1] || "";

  let melhor = null;
  let melhorScore = 0;

  for (const fixture of jogosAoVivo) {
    const homeName = normalizarNome(fixture.teams?.home?.name || "");
    const awayName = normalizarNome(fixture.teams?.away?.name || "");

    let score = 0;
    if (timeCasa && homeName.includes(timeCasa)) score += 2;
    if (timeCasa && timeCasa.includes(homeName.split(" ")[0])) score += 1;
    if (timeFora && awayName.includes(timeFora)) score += 2;
    if (timeFora && timeFora.includes(awayName.split(" ")[0])) score += 1;
...
// Extrai valor de uma estatística por nome
function extrairStat(stats, teamIndex, nomeStat) {
  const team = stats[teamIndex];
  if (!team) return null;
  const item = (team.statistics || []).find(s =>
    (s.type || "").toLowerCase().includes(nomeStat.toLowerCase())
  );
  if (!item) return null;
...
// Formata eventos em texto legível
function formatarEventos(eventos) {
  if (!eventos || eventos.length === 0) return "";
  return eventos.map(ev => {
    const min   = ev.time?.elapsed ? `${ev.time.elapsed}'` : "";
    const tipo  = ev.type  || "";
    const detalhe = ev.detail || "";
    const jogador = ev.player?.name || "";
    const time  = ev.team?.name || "";

    if (tipo === "Goal")         return `⚽ ${min} GOL de ${jogador} (${time}) — ${detalhe}`;
    if (tipo === "Card")         return `🟨 ${min} Cartão ${detalhe} para ${jogador} (${time})`;
    if (tipo === "subst")        return `🔄 ${min} Substituição: ${jogador} entra (${time})`;
    if (tipo === "Var")          return `📺 ${min} VAR: ${detalhe}`;
    return `${min} ${tipo}: ${detalhe} — ${jogador}`;
  }).filter(Boolean).join("\n");
}

// Preenche os campos do painel com dados da API
function preencherCampos(fixture, stats, eventos) {
  const elapsed = fixture.fixture?.status?.elapsed || "";
  const golsCasa = fixture.goals?.home ?? 0;
  const golsFora = fixture.goals?.away ?? 0;

  document.getElementById("live-minuto").value      = elapsed;
  document.getElementById("live-gols-casa").value   = golsCasa;
  document.getElementById("live-gols-fora").value   = golsFora;

  if (stats && stats.length >= 2) {
    const posse_c  = extrairStat(stats, 0, "Ball Possession");
    const posse_f  = extrairStat(stats, 1, "Ball Possession");
    const chut_c   = extrairStat(stats, 0, "Total Shots");
    const chut_f   = extrairStat(stats, 1, "Total Shots");
    const alvo_c   = extrairStat(stats, 0, "Shots on Goal");
    const alvo_f   = extrairStat(stats, 1, "Shots on Goal");
    const esc_c    = extrairStat(stats, 0, "Corner Kicks");
    const esc_f    = extrairStat(stats, 1, "Corner Kicks");
    const amar_c   = extrairStat(stats, 0, "Yellow Cards");
    const amar_f   = extrairStat(stats, 1, "Yellow Cards");

    if (posse_c)  document.getElementById("live-posse-casa").value       = posse_c;
    if (posse_f)  document.getElementById("live-posse-fora").value       = posse_f;
    if (chut_c)   document.getElementById("live-chutes-casa").value      = chut_c;
    if (chut_f)   document.getElementById("live-chutes-fora").value      = chut_f;
    if (alvo_c)   document.getElementById("live-chutesalvo-casa").value  = alvo_c;
    if (alvo_f)   document.getElementById("live-chutesalvo-fora").value  = alvo_f;
    if (esc_c)    document.getElementById("live-escanteios-casa").value  = esc_c;
    if (esc_f)    document.getElementById("live-escanteios-fora").value  = esc_f;
    if (amar_c)   document.getElementById("live-amarelos-casa").value    = amar_c;
    if (amar_f)   document.getElementById("live-amarelos-fora").value    = amar_f;

    document.getElementById("live-auto-badge").style.display = "inline-flex";
  }

  if (eventos && eventos.length > 0) {
    document.getElementById("live-eventos").value = formatarEventos(eventos);
  }
}

// Mostra status da busca no painel
function mostrarApiStatus(tipo, msg) {
  const el = document.getElementById("live-api-status");
  if (!el) return;
  el.style.display = "flex";
  el.className = `live-api-status live-api-status-${tipo}`;
  el.innerHTML = `
    <span>${tipo === "ok" ? "✅" : tipo === "erro" ? "❌" : "🔄"}</span>
    <span>${msg}</span>`;
}

// Função principal: busca dados e analisa
async function buscarDadosEAnalisar() {
  const rapidApiKey = document.getElementById("rapidapi-key").value.trim();
  const nomeJogo    = document.getElementById("live-jogo").value.trim();

  if (!nomeJogo) { alert("Informe o nome do jogo."); return; }
  ...
  mostrarApiStatus("loading", "Buscando dados em tempo real da API-Football...");

  try {
    if (rapidApiKey) {
      // Tenta buscar da API
      const jogosAoVivo = await buscarJogosAoVivo(rapidApiKey);

      if (jogosAoVivo.length === 0) {
        mostrarApiStatus("erro", "Nenhum jogo ao vivo encontrado agora. Verifique se o jogo já começou.");
      } else {
        const fixture = encontrarJogo(jogosAoVivo, nomeJogo);

        if (!fixture) {
          const nomes = jogosAoVivo.slice(0, 5).map(f =>
            `${f.teams?.home?.name} x ${f.teams?.away?.name}`).join(", ");
          mostrarApiStatus("erro", `Jogo não encontrado. Jogos ao vivo agora: ${nomes}. Verifique o nome.`);
        } else {
          ultimoFixtureId = fixture.fixture?.id;
          ultimoJogoNome  = nomeJogo;

          const [stats, eventos] = await Promise.all([
            buscarEstatisticas(rapidApiKey, ultimoFixtureId),
            buscarEventos(rapidApiKey, ultimoFixtureId)
          ]);

          preencherCampos(fixture, stats, eventos);
          ...
          mostrarApiStatus("ok",
            `✅ Dados atualizados! ${homeName} ${gh} x ${ga} ${awayName} — ${elapsed}' | ID: ${ultimoFixtureId}`);

          // Inicia auto-update a cada 60s
          iniciarAutoUpdate();
        }
      }
    } else {
      mostrarApiStatus("erro", "Chave RapidAPI não informada. Preencha os dados manualmente.");
    }
  } catch(e) {
    mostrarApiStatus("erro", `Erro: ${e.message}`);
  } finally {
    btn.disabled = false;
    btn.innerHTML = `🔴 Buscar Dados e Analisar com IA`;
  }

  // Sempre analisa com a IA após buscar (ou com dados manuais se falhar)
  await analisarAoVivo();
}

// Atualiza apenas os dados sem re-analisar (para o countdown)
async function atualizarDadosSilencioso() {
  const rapidApiKey = document.getElementById("rapidapi-key").value.trim();
  if (!rapidApiKey || !ultimoFixtureId) return;

  try {
    const [jogosAoVivo, stats, eventos] = await Promise.all([
      buscarJogosAoVivo(rapidApiKey),
      buscarEstatisticas(rapidApiKey, ultimoFixtureId),
      buscarEventos(rapidApiKey, ultimoFixtureId)
    ]);

    const fixture = jogosAoVivo.find(f => f.fixture?.id === ultimoFixtureId);
    if (fixture) {
      preencherCampos(fixture, stats, eventos);
      const elapsed = fixture.fixture?.status?.elapsed || "?";
      const gh = fixture.goals?.home ?? 0;
      const ga = fixture.goals?.away ?? 0;
      mostrarApiStatus("ok",
        `🔄 Atualizado às ${new Date().toLocaleTimeString("pt-BR")} — ${elapsed}' · ${gh} x ${ga}`);
    }

    // Re-analisa com IA após atualizar dados
    await analisarAoVivo();

  } catch(e) {
    mostrarApiStatus("erro", `Erro ao atualizar: ${e.message}`);
  }
}

// Inicia o ciclo de atualização automática a cada 60s
function iniciarAutoUpdate() {
  pararAutoUpdate(); // limpa qualquer anterior

  const bar = document.getElementById("auto-update-bar");
  if (bar) bar.style.display = "flex";

  let segundos = 60;
  const countdown = document.getElementById("countdown");

  autoUpdateCountdown = setInterval(() => {
    segundos--;
...
function pararAutoUpdate() {
  ...
  if (autoUpdateCountdown) { clearInterval(autoUpdateCountdown); autoUpdateCountdown = null; }
  const bar = document.getElementById("auto-update-bar");
